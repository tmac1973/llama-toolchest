...
//
// backend is the backend of the build the router launches with ("rocm",
// "cuda", ...); it selects the device-name prefix for per-model GPU
// restrictions (see gpuPlacementParams). "" emits the legacy padded
// tensor-split instead.
func GeneratePresetINI(modelsDir string, models []*Model, configs map[string]*ModelConfig, backend string) string {
	...
	for _, m := range models {
		...
		writeConfigParams(&b, cfg, isEmbed, backend)
		b.WriteString("\n")
	}
...
func writeConfigParams(b *strings.Builder, cfg *ModelConfig, isEmbedding bool, backend string) {
...
// WritePresetINI generates and writes the preset INI file to the data
// directory. backend is the launching build's backend (see
// GeneratePresetINI).
func (r *Registry) WritePresetINI(backend string) (string, error) {
	return r.writePreset(PresetFileName, nil, backend)
}
...
func (r *Registry) WriteBenchPresetINI(overrides map[string]*ModelConfig, backend string) (string, error) {
	return r.writePreset(BenchPresetFileName, overrides, backend)
}

func (r *Registry) writePreset(name string, overrides map[string]*ModelConfig, backend string) (string, error) {
	r.mu.RLock()
	...
	content := GeneratePresetINI(r.modelsDir, models, configs, backend)
